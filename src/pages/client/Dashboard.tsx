...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    // A. FETCH BASIC STATS
    try {
      const res = await fetch(`http://localhost:5000/api/auth/stats/${realId}`);
      if (res.ok) {
    ...
    try {
        const resNotes = await fetch(`http://localhost:5000/api/auth/notifications/${realId}`);
        if (resNotes.ok) {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/auth/checkin', {
        method: 'POST',
