...
export default function AdminMembership() {
  ...
  const handleSeed = async () => {
      ...
      await fetch('http://localhost:5000/api/admin/seed-plans', { method: 'POST' });
      fetchPlans();
  };

  const handleDelete = async (id: string) => {
      if(!window.confirm("Remove this plan?")) return;
      await fetch(`http://localhost:5000/api/admin/delete-plan/${id}`, { method: 'DELETE' });
      fetchPlans();
  };

  const handleAdd = async () => {
      const featuresArray = newPlan.features.split(',').map(s => s.trim());
      await fetch('http://localhost:5000/api/admin/add-plan', {
          method: 'POST',
